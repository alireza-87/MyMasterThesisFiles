...
\label{sect:sudoexp}
Its great importance is not hidden from anyone, sudo stood for \emph{supper user do}. As the name suggests, it is an application for unix\-like operating system that allows users to execute a program under a root user privilege. The \lstinline{Qyalys} \footnote{\href{https://blog.qualys.com/vulnerabilities-threat-research/2021/01/26/cve-2021-3156-heap-based-buffer-overflow-in-sudo-baron-samedit}{\texttt{CVE-2021-3156}}} researcher recently found a \emph{heap overflow} vulnerability inside the sudo. This vulnerability causes that a normal privileged user gets root privileges without authorization. It sounds dangerous mainly when we know the vulnerability available in many major unix\-like operating systems like MacOs. First, we argue how they found it while no one else could for almost decades, and then, we introduced the technical detail about exploitation.
