...
Every platform has a way to interact with heap memory. Our work specifically on \emph{ \libc{}} memory allocator which drives from \emph{ptmalloc} heap implementations, which is itself derived from \emph{dlmalloc}, so, at first we want to discuss \libc{} allocations algorithms and Methods. Consider the following code as an allocation sample:
...
The returned address by \libc{} is not the address of chunk, but, the address of the data section. if We minus these two numbers we can see 16 bytes different which use for metadata. If you look at the chunk data structure, there are two \lstinline{INTERNAL_SIZE_T} is used to keep the size of the chunk. The \libc{} documentation said ' \lstinline{|INTERNAL_SIZE_T} is the word-size used for internal bookkeeping of chunk sizes'. The default size is equal to \sizet{}. Now if we take a look at inside the memory:
...
\begin{enumerate}
	\item A (\lstinline{NON_MAIN_ARENA}): 0 for chunks in the main arena
	\item M (\lstinline{IS_MMAPPED}): The chunk is obtained through mmap
	\item P (\lstinline{PREV_INUSE}):
\end{enumerate}
...
\glnote{what is 0x555555559010? why all three blocks point to it?IT IS ALMOST THE HEAP ADDRESS , THE BIN IN THIS SAMPLE IS SINGLE LINK LIST }
...
\begin{enumerate}
	...
	\item If all the above space failed, then malloc returns NULL, which means we cannot allocate new space.
\end{enumerate}

\subsection{Allocation from Freed Chunks}
As we mention, Previously freed chunks are the first place search by the heap manager. Heap manager tracks of Freed chunks via some linked list call \emph{bins} when a new allocation request arrives; the heap manager looks at bins to find a big enough chunk. If the chunk has the same size, then the heap manager returns it. Otherwise, if the chunk is more significant than the request, then the heap manager split it. There are several different types of bins which we discuss later.

\subsection{Allocation from Top Chunk}
So what does happen if there were no suitable previously free space? In this situation, the heap manager checks the remaining space at the end of the heap. If there is enough space to create a chunk, then allocate a new chunk from the top of the heap space.

\subsection{Ask Kernel for more memory}
So, let us check what happens yet—first, the heap manager search through previously free chunk. Assume we do not have any suitable free chunk. As a result, the heap manager performs chunk creation from the top chunk; however, the request is bigger than the remaining space. If there is not enough space at the end of the heap, the heap manager asks Linux Kernel for more memory. Now we have to discuss another system called \emph{brk}. When a program executes, the heap manager calls \emph{sbrk} which used the brk system call. This system call allocates memory just after the program gets load which is the end of the heap.
After a while, this system calls \lstinline{break} because the newly allocated region at the end of the program space collides with another thing in the process's space. Once this situation happens, the heap manager cannot allocate contiguous memory space by calling mmap to allocate non-contiguous memory space.

\section{MMAP}
The heap manager uses \emph{mmap} for large memory allocation instead of \lstinline{sbrk} If the user requests a too large memory space. Chunks metadata contain a special flag for these reasons, which indicate this chunk allocated off-heap via mmap. After the user releases the memory by \lstinline{free()}, these chunks are \emph{unmmap}. By default, the \lstinline{mmap} threshold is 128KB to 512KB for 32-bit and 32MB for the 64-bit system.
...
To improve the performance, \lstinline{ptmalloc2} introduces a concept called \emph{arena}. Different threads have their arena; also, Each arena manages its chunks. As a result, threads can work simultaneously in the different arenas without altering each other data.

When the process creates a new thread, the heap manager finds a new arena up to the maximum allowed number. The default value from the maximum number of the arena is (2* the number of CPU core) for 32-bit and (8* the number of CPU core) for 64-bit; also, you can tune this value by \emph{mallopt} function. So, what happens when we reach max number? Thread has to share the arena.
\glnote{MALLOPT WILL ADD - DONE}

As we saw before, the main Arenas create and grow by the \lstinline{sbrk} system call; however, this is not true for the second heap. The second heap creates via \lstinline{mmap}.

\section{Subheap}
The main heap arena is located just after where the program started and expanded by \lstinline{sbrk}. We can conclude that in the multi-thread application, which has a secondary heap, \lstinline{sbrk} can not do this. This secondary arena emulates the main heap arena by using \emph{subheap}. subheap works like the main heap; however, they have some differences. The main heap is located right after the program starts in memory; they grow with the \lstinline{sbrk} system call. Subheap can not contiguously allocate after the program begins because it creates by mmap; moreover, the heap manager expands them by 'protect.

How heap manager create and manage the subheap? In the first step, the heap manager asks the Kernel to reserve an area of memory for the \emph{subheap} by malloc. Reserving does not allocate memory to subheap; it just tells the Kernel, Do not use this area. Mmap by flags required page as \lstinline{PROT_NONE} achieve this goal.
On the next step, when the main heap expands by \lstinline{sbrk}, the heap manager allocates the previously reserved area to subheap by calling \lstinline{mprotect}. What \lstinline{mprotect} do? It concerts \lstinline{PROT_NONE}\glnote{use lstinline for identifiers/function-names/etc; change everywhere} to \lstinline{PROT_READ}, \lstinline{PROT_WRITE}. In this way, at least the Kernel allocates real physical memory to each subheap, subheap expand in mmap area until It fills all mmap reserve area.
...
As we can see, malloc iterate over all items inside \ub{}. We can conclude that when the heap manager tries to find chunks in \ub{}, all of the \ub{} chunks move to corresponding bins.
...
\section{malloc()}
Let us put all it together to find out how malloc works. When a new allocation request arrives, malloc does the following procedure:
...
This function checks if the system supports mmap allocation and the requested size is inside the mmap threshold. It allocates memory by using mmap. If it fails means there is no memory anymore, so malloc returns a null pointer.

\section{calloc()}
Calloc \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l3365}{\texttt{malloc.c}, line 3365}} is another glib function used to allocate memory. Calloc and malloc have one important difference. Unlike malloc, calloc does not use \tch{}. If we have free chunks in \tch{}, then request a new memory by calloc, the heap manager will not use them \glnote {DONE do not use contractions in formal writing; in this case, "will not" (instead of will not)}. Instead, create a new chunk from the top chunk. consider the following sample
...
If we look at the heap state after calloc, we can see heap manager create new chunks from the top chunk and \tch{} chunks remain.
...
At first, we requested 3 chunks of 10-byte. After that, we free \lstinline{a}\glnote{instead of single quotes, use lstinline for a, b, etc - DONE} and \lstinline{b} and at last \lstinline{a} again. You can see the fast bin structure after \lstinline{free()} below step by step:
...
The thirds \lstinline{malloc} have a significant memory allocation; this allocation triggers another method call \lstinline{malloc_consolidate()} automatically. \lstinline{malloc} In this situation, consolidate chunks in the fast bin before continuing, then put the resulting merged chunks on the \ub{}.
Such behavior is that we have another solution to bypass the fast bin double-free security check. It is possible to use \lstinline{malloc} with a big size instead of free another chunk in the middle of two free. The third \lstinline{malloc} moves chunk from fast bin to unsorted; as a result, when the second free request arrives, there is no chunk inside the \fb{}. We can implement a double-free attack. In the final layout, we have \lstinline{P1} on both \fb{} and \ub{}. Move \fb{} chunks to \ub{} by a big size memory allocation is a simple approach which uses by other attack methods too.
...
Now let us use double-free vulnerabilities in \libc{-2.23}; the below code is just an extended version of the Double Free attack which we examined. The extended version tricks \lstinline{malloc()} to return a controller memory location on the stack instead of a real chunk of the heap.
...
The final step is to write 0x20 on the stack, overwrite the first 8 bytes of data at \lstinline{a} (any change on \lstinline{d} affect \lstinline{a} at the top of the \fb{}) in an away point to right before 0x20 on the stack. In this way, we trick the heap into thinking there is an available free chunk.
 Now, all we have to do is remove \lstinline{a} from the top of the \fb{} with a call to \lstinline{malloc()}, then, the final allocation returns a controller address on memory to the program is stack in this case.
...
The last \lstinline{malloc} tries to allocate 400-Byte of memory, so the heap manager checks the \ub{} to find a fair chunk of memory. Also, move the \ub{} chunk to corresponding bins. If we check the \lstinline{malloc} source code, we can see \lstinline{malloc} check every item to achieve this goal. At first, we have such a code \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l3728}{\texttt{malloc.c}, line 3728}} :
\begin{lstlisting}[language=c,frame=tlrb]

while ((victim = unsorted_chunks (av)->bk) != unsorted_chunks (av)){
bck = victim->bk;
\end{lstlisting}
‘Av’ is arena and \lstinline{unsorted_chunks (av)} is the head of the \ub{} and Victim is our current item. You can see, \lstinline{malloc} checks items from the head of the \ub{} and removes them one by one. Put it all to gather:
...
This attack abuses the vulnerability inside \sbs{} to modify memory in any location. If we check the \lstinline{malloc} source code, we can see there is a check for small chunk request, so, if the requested chunk fits inside the \sbs{} range, the heap manager use it.
...
\begin{figure}[h!]
 ...
 \caption{Heap layout}
\end{figure}
The victim is not in the \ub{} anymore; instead, the heap manager moves it to the \sbs{} after allocation of p2. As we remember, the heap manager checks all of the chunks inside the \ub{}. If the size is not suitable for the current allocation, they move to the corresponding bin. In other words, we force the heap manager to move the victim from the \ub{} to the \sbs{}.
...
The result of such an action is, our fake chunk is at the top of the bin, so the next \lstinline{malloc} return to the attacker's controlled location.

\subsection{The House of Orange}
The house of orange is different from previous attacks. As we remember, in the previous attack, we use \lstinline{malloc} and free to abuse libc vulnerability, but, In the house of orange, we do not use the free() function at all. In other words, we force the heap manager to free a chunk of the heap without calling the free() function. The final layout of this attack is a combination of \ub{} attacks; also, in the end, we gain shell access.
...
As you can see, libc keeps file pointers inside \lstinline{_io_list_all}, so, in the corruption situation, all pointers be flush by the use of the \lstinline{io_file} object. When libc allocate a new file structure and fopen() return the pointer, the actual internal structure inside the \libc{} is \lstinline{io_file_plus} , if we take a look at \libc{} code we have:
...
The first step is to get a pointer inside \emph{\lstinline{malloc_hook}}, but what is the \lstinline{malloc_hook}? \lstinline{malloc_hook} lets us modify the behavior of \lstinline{malloc} and \lstinline{free} with a hook function. To achieve this, we use a \fb{} attack in a way the \lstinline{fd} pointer points to \lstinline{malloc_hook}.

\subsection{The House of Rabbit}

This attack used a \Fb{} and \lstinline{UAF} attack to forge a fake heap chunk for feature uses. As we mentioned before, every fast bin keeps a single size chunk with a single linked list. What happens if we change a chunk inside the \fb{}? In allocation, \fb{} detects the abnormal size so aborts the allocation. If we force the heap manager to call \lstinline{malloc} inorder to consolidate, this chunk merge and split into \sbs{}. as a result, we have two valid but forged chunks in a \sbs{} which overlap which other. Consider the following code:
...
In this attack, the goal is to force \lstinline{malloc} to return a chunk in any given address by use of the free function. Consider the free function:
...
The final step is free \lstinline{b} after free \lstinline{b} it is merged without fake chunk, so next \lstinline{malloc} returns our fake chunk.
...
The above procedure has another result, too; now we have two chunks that overlap too. For the rest of the attack, we perform a \tch{} poisoning. First, define a victim and overwrite the fd pointer pointing to the target, then two allocations. The last allocation has control over the target.
