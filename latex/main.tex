...
Over the years, a lot of works has been done to make it more secure. In addition, many techniques have been used to exploit the weaknesses of its various implementations. In this section, we mention some of the previous work on heap exploitation. In \refToSection{heaphopper} we discuss heap hopper ~\cite{217488}  as a Framework to evaluate any heap implications, then in \refToSection{maze}, we introduced MAZE   ~\cite{272228} which is an Automatic heap fang Shui technique. In section \refToSection{ptauth} we discuss PTAuth~\cite{263812} .
