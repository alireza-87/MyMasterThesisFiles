...
Now, all we have to do is remove \lstinline{a} from the top of the \fb{} with a call to \mallocc{}, then, the final allocation returns a controlled address. In this case, a pointer into the stack.\glnote{DONE-I rephrased, please check if this is what you meant}
...
Let's start with an example and consider the following code. This code tries to write an address inside the heap area in the stack.\glnote{DONE-I don't understand in which sense is trying to write ``a big'' value\ldots} In \refToSection{houseofroman}, we talk about using this type of vulnerability in a real attack.
...
Now it is clear that if we control the backward pointer, then, we can write data to any address, however, we do not have control over data. In this case, the linked list header was written on a stack variable.In the case of \emph{ASLR}, this vulnerability can help to find an address inside the heap area.\glnote{DONE-this could be useful to defeat ASLR! You should say so}
