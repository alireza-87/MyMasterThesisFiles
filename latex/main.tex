...
free(array);
\end{lstlisting}
The \lstinline{malloc} does not clear the allocated memory, so it contains junk data. In order to allocate memory and zero it at the same time, it is possible to use the calloc() function.
\begin{lstlisting}[language=c,frame=tlrb]
...
\section{calloc()}
Calloc \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l3365}{\texttt{malloc.c}, line 3365}} is another glib function used to allocate memory. Calloc and \lstinline{malloc} have one important difference. Unlike \lstinline{malloc}, calloc does not use \tch{}. If we have free chunks in \tch{}, then request a new memory by calloc, the heap manager will not use them. Instead, create a new chunk from the top chunk. consider the following sample
