...
\end{lstlisting}
The above code computes the number of bytes required to store 20 integers in memory, then requests that number of bytes from \lstinline{malloc}\glnote{use lstinline for identifier in the text; I added some, check everywhere}. Function \lstinline{malloc} returns the pointer to that ten\glnote{ten?} bytes in the memory. Sometimes may \lstinline{malloc} cannot allocate memory; in such a scenario, it returns a null pointer.
\begin{lstlisting}[language=c,frame=tlrb]
if (array == NULL) {
...
\end{lstlisting}
After we finish the job and do not need the array anymore, it is possible to release the memory with the free() function.\glnote{I would not use ``()'', just ``function \lstinline{free}''}
\begin{lstlisting}[language=c,frame=tlrb]
free(array);
\end{lstlisting}
The \lstinline{malloc} does not clear the allocated memory, so it contains junk data. In order to allocate memory and zero it at the same time, it is possible to use the \lstinline{calloc} function.
\begin{lstlisting}[language=c,frame=tlrb]
...
This allocation metadata and padding are stored alongside the allocated memory, which \lstinline{malloc} returns to the user. For this reason, the heap manager uses a data-structure called \emph{chunk}, each chunk is usually bigger than request memory allocation, because of metadata and alignment. When the user sends a request for memory allocation, the heap manager finds a chunk which big enough for user data plus metadata, then, returns a pointer to the user's data section of the chunk. The code below shows the chunk data structure \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;\#l1163}{\texttt{malloc.c}, line 1163}}:
...
\emph{The memory area used for these pointers overlaps user's data, returned by \lstinline{malloc}}.
...
\begin{enumerate}
	...
	\item M (\lstinline{IS_MMAPPED}): was the chunk allocated through \lstinline{mmap}?
...
\begin{enumerate}
	...
	\item If all the above fails, then \lstinline{malloc} returns \lstinline{NULL}, which means we cannot allocate the requested memory.
\end{enumerate}
...
\section{MMAP}
The heap manager uses \emph{mmap}\glnote{use lstinline not emph for function names}, instead of \lstinline{sbrk}, also for large memory allocations. That is, when the user requests a size larger than a certain threshold. Chunks metadata contain a special flag for these reasons, which indicate this chunk allocated off-heap via mmap. When the user releases the memory by calling \lstinline{free()}, these chunks are \lstinline{unmmap}ed. By default, the \lstinline{mmap} threshold is 128KB to 512KB\glnote{which one? I don't understand} for 32-bit and 32MB for the 64-bit system.
...
To improve the performance \emph{ptmalloc2}\glnote{in this case emph, because it's the name of the library, not an identifier} introduces a concept called \emph{arena}. Different threads may have their own arena, and each arena manages its chunks. As a result, threads can work simultaneously in the different arenas without the need to synchronize their access to a shared data structure.
...
As we saw before, the main arena grows by using the \lstinline{sbrk} system call; however, this is not true for secondary arenas, which grow by calling \lstinline{mmap}.
...
\section{Free}
Whats happens when the user does not need the allocated space anymore? By calling \lstinline{free}, the heap can free up the space. Now, the question is how free() works? First, it needs to calculate the chunk address from the user pointer. The user has a pointer to the user data section of the chunk, but free() needs the actual address of the chunk, so, by subtracting the pointer address from metadata size, we can find the chunk address. Sending an invalid pointer may lead to memory corruption; as a result, the heap manager does some security checks before freeing up space:
...
\section{Bins}
	The heap manager requires a mechanism to manage and organize recently freed chunks for the following allocations. The simplest solution to keep track of freed space is to store them in a giant bin; although this could work, it is not a good solution. \lstinline{Malloc} uses vary wildly, so this approach leads to a performance problem.
To improve performance, the heap manager uses different types of bins. There are five different type of bins: \emph{\sbs{}}, \emph{\lb{}}, \emph{\fb{}}, \emph{\ub{}} and \emph{\tch{}}. All of the mentioned bins exist in the same area of heap manager source code.\glnote{source code? Do you mean that they are allocated together in the source code? If this is the case, maybe you can add the snippet of code here to show what you mean} This is a list with 127 items, the index 0 is unused.
...
As we can see, \lstinline{malloc} iterate over all items inside \ub{}. We can conclude that when the heap manager tries to find chunks in \ub{}, all of the \ub{} chunks move to corresponding bins.
...
\section{malloc}
Let us put all it together to find out how \lstinline{malloc} works. When a new allocation request arrives, \lstinline{malloc} does the following procedure:
...
This function checks if the system supports mmap allocation and the requested size is inside the mmap threshold. It allocates memory by using mmap. If it fails means there is no memory anymore, so \lstinline{malloc} returns a null pointer.

\section{calloc}
\lstinline{Calloc} \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l3365}{\texttt{malloc.c}, line 3365}} is another glib function used to allocate memory. \lstinline{Calloc} and \lstinline{malloc} have one important difference. Unlike \lstinline{malloc}, \lstinline{calloc} does not use \tch{}. If we have free chunks in \tch{}, then request a new memory by \lstinline{calloc}, the heap manager will not use them. Instead, create a new chunk from the top chunk. consider the following sample
...
If we look at the heap state after \lstinline{calloc}, we can see heap manager create new chunks from the top chunk and \tch{} chunks remain.
...
\section{mallopt()}
mallopt is a function used to tune the behavior of \libc{} memory allocation methods like \lstinline{malloc}. In the below, we mention the parameters list, which mallopt can modify.
...
To find when \lstinline{malloc_consolidate()} \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l4440}{\texttt{malloc.c}, line 4440}} calls, we invest two different versions of \libc{}. the first one is 2.23 and the second one is 2.33 which is the latest \libc{} version. The first usage is the \lstinline{malloc} function , Consider the following code:
...
As we tell, small/\lb{} are one of the places which \lstinline{malloc} search to find free chunk at the first phase. During this check, the heap manager performs flush to move \fb{} chunk too. If we check mentioned section in the latest \libc{} version, we see a \sbs{} check does not contain \lstinline{malloc_consolidate} anymore \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l3635}{\texttt{malloc.c}, line 3635}} .
...
The second usage is when the heap manager wants to use the top chunk in \lstinline{malloc}. consider the following code in \lstinline{malloc} \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l4128}{\texttt{malloc.c}, line 4128}}:
