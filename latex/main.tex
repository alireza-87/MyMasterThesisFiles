...
In the \refToChapter{introheap} we talk about heap, then, In \refToChapter{heapexp} we introduce various type of attack on \libc{} heap. The question may arise when the heap memory can be used as an option to attack the system in the real world. To answer this question, we examine several attacks on the heap and their obstacles and problems. In \refToSection{sudoexp} we Introduce one of the most recent attacks on the sudo, then, In \refToSection{whatsappexp} we discuss an attack on one of the most famous messenger.

\section{Heap-Based Buffer Overflow in Sudo}
\label{sect:sudoexp}
Its great importance is not hidden from anyone, \lstinline{sudo} stood for \emph{supper user do}. As the name suggests, it is an application for \lstinline{unix-like} operating system that allows users to execute a program under a root user privilege. The \lstinline{Qyalys} \footnote{\href{https://blog.qualys.com/vulnerabilities-threat-research/2021/01/26/cve-2021-3156-heap-based-buffer-overflow-in-sudo-baron-samedit}{\texttt{CVE-2021-3156}}} researcher recently found a \emph{heap overflow} vulnerability inside the \lstinline{sudo}. This vulnerability causes that a normal privileged user gets root privileges without authorization. It sounds dangerous mainly when we know the vulnerability available in many major \lstinline{unix-like} operating systems like \lstinline{MacOs}. First, we argue how they found it while no one else could for almost decades, and then, we introduced the technical detail about exploitation.

Although it may not be entirely relevant to the topic of the thesis, the first question that arises is how they discovered the bug. It may lead to understanding the \lstinline{sudo} better. \emph{Fuzzing} is an automatic software testing that provides invalid input data to target programs to monitor program behavior. \lstinline{Afl} stands for American fuzzer looper is a free fuzzer software that employs a genetic algorithm to generate the best-unexpected input for the program. These two play a crucial role in found software vulnerability. \lstinline{Sudo} is not just a program as the others; so, we need some modification to test \lstinline{sudo}. \lstinline{Afl} read input data from a file and pass it as an argument. It would not fuzz argument by default. To fuzz arguments of a program, \lstinline{afl} developers provide a header file.
