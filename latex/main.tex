...
0x555555559300:	0x0000000000000000	0x0000000000000000
\end{lstlisting}
\glnote{DONE-NOT SURE ABOUT THIS .... -what is 0x555555559010? why all three blocks point to it? }
...
As we saw before, the main arena grows by using the \sbrkc system call; however, this is not true for secondary arenas, which grow by calling \mmapc{}.
...
\end{enumerate}
\glnote{DONE, THE FREE REWRITE AND MORE DETAIL ADDED LIKE MALLOCyou should expand point 5, by describing the logic/policy of how bins are selected/used (similarly to what you did for malloc in section 2.8, linking the corresponding section; moreover, it's probably better to re-order the section in the same order}
\glnote{2.7 (bins) should be probably a subsection of 2.6? and, in turn, its subsections would become subsubsections and so on}
...
\subsection{\Fb}
Each \Fb{} keep recently freed chunks in a single-linked list. We have 10's \fb{}, each of them keeps just one chunk size. Chunks in \fb{} do not merge with neighbors; instead, \fb{} keeps them alive, so, if the program sends the same size request, then the heap manager can use these chunks.\glnote{DONE-I DO SOME REPHRASES - I am not sure I understand}
...
The main goal of this attack is to get shell access by a combination of \ub{} attack and Use after the free attack.
...
As we talk before, the unsorted bin attack let us write a value in a controlled location. The final goal of the attack is to get the shell access; to do this, we need to have the libc address.
