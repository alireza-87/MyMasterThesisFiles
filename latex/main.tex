...
The result of the above is to consolidate the \lstinline{prev} chunk with the victim \lstinline{a}\glnote{done-Again, use lstinline for identifiers, not single quotes; be consistent} ``Figure~\ref{fig:gdb11}``. Attention to the size and the address of the chunk:
...
The above procedure has another result, too; now we have two chunks that overlap too. For the rest of the attack, we perform a \tch{} poisoning. First, define a victim and overwrite the \lstinline{fd} pointer pointing to the target, then two allocations. The last allocation has control over the target.``Figure~\ref{fig:gdb12}``
...
In the \refToChapter{introheap} we talk about heap, then, In \refToChapter{heapexp} we introduce various type of attack on \libc{} heap. The question may arise , when the heap memory can used as an option to attack the system in the real world. In order to answer this question, I will examine a number of attacks on the hip and its obstacles and problems. In \refToSection{sudoexp} we Introduce one of the must recent attack on the sudo, then, In \refToSection{whatsappexp} we discuss about an attack on one of the must famous messenger.
