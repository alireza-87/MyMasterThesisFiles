...
\end{lstlisting}
The above code computes the number of bytes required to store 20 integers in memory, then requests that number of bytes from \mallocc{} \glnote{use lstinline for identifier in the text; I added some, check everywhere}. Function \mallocc{} returns the pointer to that ten\glnote{ten?} bytes in the memory. Sometimes may \mallocc{} cannot allocate memory; in such a scenario, it returns a null pointer.
\begin{lstlisting}[language=c,frame=tlrb]
if (array == NULL) {
...
free(array);
\end{lstlisting}
The \mallocc{} does not clear the allocated memory, so it contains junk data. In order to allocate memory and zero it at the same time, it is possible to use the \lstinline{calloc} function.
\begin{lstlisting}[language=c,frame=tlrb]
