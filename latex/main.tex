...
\end{lstlisting}
This file work on \libc{-2.31} which support \tch{} as the first mechanism, so, like \lstinline{double free} in \refToSection{doublefree}, all we need is to fill up the \tch{} to force the heap manager role back to the \fb{}
...
\label{sect:sudoexp}
Its great importance is not hidden from anyone, sudo stood for \emph{supper user do}. As the name suggests, it is an application for \lstinline{unix-like} operating system that allows users to execute a program under a root user privilege. The \lstinline{Qyalys} \footnote{\href{https://blog.qualys.com/vulnerabilities-threat-research/2021/01/26/cve-2021-3156-heap-based-buffer-overflow-in-sudo-baron-samedit}{\texttt{CVE-2021-3156}}} researcher recently found a \emph{heap overflow} vulnerability inside the sudo. This vulnerability causes that a normal privileged user gets root privileges without authorization. It sounds dangerous mainly when we know the vulnerability available in many major \lstinline{unix-like} operating systems like \lstinline{MacOs}. First, we argue how they found it while no one else could for almost decades, and then, we introduced the technical detail about exploitation.

Although it may not be entirely relevant to the topic of the thesis, the first question that arises is how they discovered the bug. It may lead to understanding the sudo better. \emph{Fuzzing} is an automatic software testing that provides invalid input data to target programs to monitor program behavior. \lstinline{Afl} stands for American fuzzer looper is a free fuzzer software that employs a genetic algorithm to generate the best-unexpected input for the program. These two play a crucial role in found software vulnerability. Sudo is not just a program like the others; so, we need some modifications to test sudo. \lstinline{Afl} read input data from a file and pass it as an argument. It would not fuzz argument by default. To fuzz arguments of a program, \lstinline{afl} developers provide a header file.
...
This function reads our fake \lstinline{argv} from an input file, then, overwrites the real \lstinline{argv} with our fake \lstinline{argv}. Now we can fuzz the sudo and \lstinline{argv}; nevertheless, it would not work. The header file has a buffer overflow; therefore, a crash arises inside the \lstinline{afl} if we try to fuzz the sudo. There is another fork of \lstinline{afl} which name is \lstinline{afl++}, this implementation has non of mentioned bugs.
...
Linux provides two different \lstinline{userId}: the \lstinline{userId}, the \lstinline{effective userId}. A program like the sudoedit uses \lstinline{setUid} to set \lstinline{userId}. If we execute a program with sudo, the program gets our normal \lstinline{userId} like 1000 also gets \lstinline{effective userId} as 0 because we need to execute it as the root user. If we fuzz the sudo with \lstinline{afl} in order to interact with \lstinline{setUid} in the sudo, our fuzzer need to execute as root which runs sudo. In such a situation sudo might show different behavior because it's already root.
