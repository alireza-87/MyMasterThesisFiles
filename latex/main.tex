...
\subsection{\Fb}
\Fb{} keep recently freed small chunks in a single-linked list. As we mentioned, \ub{} merge chunks before adding them to bins. However, chunks in \ub{} do not merge with neighbors; instead, \fb{} keeps them alive, so, if the program sends the same size request, then heap manager can use this chunks.\glnote{I am not sure I understand}
