...
\glnote{This chapter is glibc-specific, so the general description should be moved to chapter 1. You could also add a general description of malloc/realloc/free, with some examples - MOVE TO C1 }An application has different types of memory space, two must important are \emph{heap} and \emph{stack}. Usually, the stack is used to store local variables, also, It may use to pass the function's parameters. Unlike stack, heap is a portion of memory which use by applications to dynamically allocate memory during run time. This means an application can request memory and freed memory during execution. The data stored in the heap are accessible by all thread, so, It is important to handle it properly. There are some other differences between stack and heap too:\glnote{As a general rule, in English there is no space before punctuation, DONE}
\begin{itemize}
...
In the above allocation, we have 16 bytes of metadata plus 8 bytes of memory allocation which total allocation becomes 24 bytes, however, \libc{} must keep memory in 16-byte alignment, as the result, the final allocation is 32 byte. Take a look at the size field in an allocated chunk, we can see the size is not 32 but 33. The reason is 3 last bits of size parse differently:\glnote{non-main-arena and prev-in-use seem swapped - DONE}
\begin{enumerate}
...
0x555555559300:	0x00000000	0x00000000	0x00000000	0x00000000
\end{lstlisting}
\glnote{ DONE - the fd/bk pointers point to the chunk user's data, right? Maybe you could point that out. I don't understand where the bk pointers are pointing in this listing... what is 0x555555559010?}
...
\glnote{DOEN - ALSO MALLOPT WILL ADD - I think these are just the defaults, they can be changed via environment variable and \texttt{mallopt}; you should mention that}
...
\section{Subheap}
As we mention, the main heap arena located just after where the program started and expanded by sbrk, so, we can conclude in the multi-thread application which has a secondary heap this can not be done. This secondary arena emulates the main heap arena by using \emph{subheap}. subheap\glnote{DONE - this is the first time you use the term subheap; clarify what it is} works like the main heap, however, they have some differences. The main heap is located right after where the program starts in memory, they grow with the sbrk system call. Subheap create by use of mmap, so, they are not located where the program starts contiguously, moreover, the heap manager expands them by ‘protect.
...
\end{enumerate}
\glnote{DONE-add link to glibc source for these checks}
...
\begin{enumerate}
	...
	\item Otherwise, if the chunk right before this chunk is free, then they merge \glnote{I EXPLAINED THE FULLY IN MALLOC AND EACH BINS SECTION , THIS IS JUST A SIMPLE PLOT WHICH MENTIONED , THEY WAY CHUNKS MERGE DEPENDED ON MANY THING WHICH EXPLAINED ON OTHER PARTS , MAY I CAN DELETE THIS SIMPLE PLOT ? the way chunks are merged should be better described; if you did this in the exploitation part, you can simply insert a reference to it}
...
\subsection{\Sbs}
There are 62 \sbs{}, Each of them stores the same size of the chunk, as the result, each list is sorted by default\glnote{I DIDNT MENTION ORDER , THE ORDER IS LIFO FOR SMALL BINS , ALSO , SMALL BINS KEEP ONE SIZE ITS SORTED BY DEFAULT  . what do you mean? I think they are inserted in a LIFO order}, also, store and retrieve chunk from \sbs{} is fast. \sbs{}  stores chunk size below 512 bytes in the 32-bit system and below 1024 Byte in 64-bit with a double-link list. 
\glnote{DONE - is this a single or double-linked list?}
...
mallopt is a function used to tune the behavior of \libc{} memory allocation methods like malloc. In the below, we mention the parameters list which can be modified by mallopt.

\begin{enumerate}
\item \lstinline{M_ARENA_MAX} Defines the maximum number of arenas that can be create. Programs usually have multiple threads, in which case an arena may receive multiple requests, so arenas must be thread-safe. Having more numbers arena means less thread concurrent, however, the higher memory needs.
...
This command provide  information about allocated and freed chunk , also , the top chunk .
...
Provide information about freed chunks inside every bins .
...
This attack works by manipulating the heap in a way a fake \lstinline{ar_ptr} supply to \lstinline{_init_free()}, as the result, an arbitrary overwrite be happen. The Attacker has to allocate enough memory until reaches \lstinline{non_main_arena} where he has control over arena data. Max heap size is 1024*1024 plus 512kb of padding, so, max allocation never is more than 1MB.
We saw It possible to hijack the \lstinline{heap_info} and supply an arbitrary \lstinline{ar_ptr} value, so, what value should we use for \lstinline{ar_ptr}? We have two option on that:
...
Now the chain of events are complete , but we have to meet the condition of  \lstinline{IO_LIST_ALL}
...
By considering the merge code of the free function we can tell if we have control over  \lstinline{prev_size} , and  \lstinline{prev_inuse} flag, we can have control over new chunk location. 
...
\end{lstlisting}
 ...
 Now its time to set  \lstinline{prev_size} of our fake chunk to an inorder to merge it with our fake chunk
