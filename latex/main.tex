...
Libc provide memory management function like malloc(),calloc(),free(). Libc manages memory dynamic, static or automatic. The static one allocates in mani memory and exists during the execution. Automatic variable allocates on the stack like function variable. The dynamic one allocates during the execution; also, dynamic variable size is unknown during the compile time. We work especially on libc dynamic memory management mechanism.Consider the following code as an allocation sample:
...
\end{lstlisting}
The above code computes the number of bytes required to store 20 integers in memory, then requests that bytes from malloc. Malloc returns the pointer to that ten bytes in the memory. Sometimes may malloc cannot allocate memory; in such a scenario, a null pointer returns. 
\begin{lstlisting}[language=c,frame=tlrb]
if (array == NULL) {
...
\end{lstlisting}
After we finish the job and do not need the array anymore, it is possible to release the memory with the free() function.
\begin{lstlisting}[language=c,frame=tlrb]
free(array);
\end{lstlisting}
The malloc() is not clear the allocated memory, so it contains junk data. In order to allocate memory and clear at the same time, it is possible to use the calloc() function.
\begin{lstlisting}[language=c,frame=tlrb]
...
On the next step, when the main heap expands by \lstinline{sbrk}, the heap manager allocates the previously reserved area to subheap by calling \lstinline{mprotect}. What \lstinline{mprotect} do? It concerts \lstinline{PROT_NONE}\glnote{DONE use lstinline for identifiers/function-names/etc; change everywhere} to \lstinline{PROT_READ}, \lstinline{PROT_WRITE}. In this way, at least the Kernel allocates real physical memory to each subheap, subheap expand in mmap area until It fills all mmap reserve area.
...
In the next sample we use double-free vulnerabilities in \libc{-2.23}; the below code is just an extended version of the Double Free attack which we examined. The extended version tricks \lstinline{malloc()} to return a controller memory location on the stack instead of a real chunk of the heap.
...
\glnote{could you clarify/rephrase the following? The role of the 0x20 is unclear, IT IS ONE OF THE FAST BINS CHUNK SIZE , I MENTIONED ON FIRST SAMPLE " It must fall in \fb{} chunks "}
...
To prevent merging the top chunk with a small one during free() we allocate a big chunk on top of the heap. After that by freeing the victim, it moves to the \ub{} ``Figure~\ref{fig:gdb2}``. In the following step, we allocate another chunk big enough, so, heap manager can't use the chunk inside an \ub{} or \sbs{}.
\begin{lstlisting}[language=c,frame=tlrb]
