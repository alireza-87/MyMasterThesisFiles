...
\section{MMAP}
The heap manager uses \mmapc{}\glnote{DONE - use lstinline not emph for function names}, instead of \lstinline{sbrk}, also for large memory allocations. That is, when the user requests a size larger than a certain threshold. Chunks metadata contain a special flag for these reasons, which indicate this chunk allocated off-heap via \mmapc{}. When the user releases the memory by calling \lstinline{free}, these chunks are \lstinline{unmmap}ed. By default, the \mmapc{} threshold is 128KB to 512KB\glnote{which one? I don't understand} for 32-bit and 32MB for the 64-bit system.
