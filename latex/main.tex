...
The question may arise when the heap memory can be used as an option to attack the system in the real world. To answer this question, we examine several attacks on the heap and their obstacles and problems. In \refToSection{sudoexp} we Introduce one of the most recent attacks on the \emph{sudo}, then, In \refToSection{whatsappexp} we discuss an attack on one of the most famous messenger.

\section{Heap-Based Buffer Overflow in Sudo}
\label{sect:sudoexp}
Its great importance is not hidden from anyone, sudo stood for \emph{supper user do}. As the name suggests, it is an application for unix\-like operating system that allows users to execute a program under a root user privilege. The \lstinline{Qyalys} ~\cite{vuln_sudo} researcher recently found a \emph{heap overflow} vulnerability inside the sudo. This vulnerability causes that a normal privileged user gets root privileges without authorization. It sounds dangerous mainly when we know the vulnerability available in many major unix\-like operating systems like MacOs. First, we argue how they found it while no one else could for almost decades, and then, we introduced the technical detail about exploitation.

Although it may not be entirely relevant to the topic of the thesis, the first question that arises is how they discovered the bug. It may lead to understanding the sudo better. \emph{Fuzzing} is an automatic software testing that provides invalid input data to target programs to monitor program behavior. Afl stands for American fuzzer looper is a free fuzzer software that employs a genetic algorithm to generate the best-unexpected input for the program. These two play a crucial role in found software vulnerability. Sudo is not just a program like the others; so, we need some modifications to test sudo. Afl read input data from a file and pass it as an argument. It would not fuzz argument by default. To fuzz arguments of a program, afl developers provide a header file.
...
This function reads our fake \lstinline{argv} from an input file, then, overwrites the real \lstinline{argv} with our fake \lstinline{argv}. Now we can fuzz the sudo and \lstinline{argv}; nevertheless, it would not work. The header file has a buffer overflow; therefore, a crash arises inside the afl if we try to fuzz the sudo. There is another fork of afl which name is afl++, this implementation has non of mentioned bugs.

The path of the vulnerability of the sudo was through the \emph{sudoedit}. The sudoedit is a symlink to actual sudo. As we now \lstinline{argv[0]} hold the filename executable, so, when we execute the sudoedit, the first argument must be sudoedit itself. This is important because the sudo has a check on the file name. The problems do not finish yet, sudoedit does not get the file name from \lstinline{argv[0]} only, it uses \lstinline{getprogname} if available. We have to remove this part from the sudo source code.

Linux provides two different \lstinline{userId}: the \lstinline{userId}, the \lstinline{effective userId}. A program like the sudoedit uses \lstinline{setUid} to set \lstinline{userId}. If we execute a program with sudo, the program gets our normal \lstinline{userId} like 1000 also gets \lstinline{effective userId} as 0 because we need to execute it as the root user. If we fuzz the sudo with afl in order to interact with \lstinline{setUid} in the sudo, our fuzzer need to execute as root which runs sudo. In such a situation sudo might show different behavior because it's already root.

After overcoming all of the mentioned problems, we can fuzz the sudo and found the problem. All of the mentioned problems were difficult to solve; therefore, we can conclude no one had done it before. That is the reason sudo overflow has not been discovered for almost a decade. The research team that found the vulnerability uses the code review to find the vulnerability.

Now, it is time to discuss how this vulnerability works. There is a loop inside the \lstinline{set_cmnd} method, which is unsecured because the pointer might become out of bounds. This code copy strings one character at a time. The problem is about the backslash check. When the string end by a backslash, the \lstinline{from} pointer move one forward, copy the next character, then move forward again. In \lstinline{c} strings must be \lstinline{null} terminate, appropriately, the \lstinline{null} byte determines the end of the string. In the Vulnerability situation, the code copy the \lstinline{null} byte to the destination mistakenly then points to an invalid location in memory. Now we have an \emph{overflow}; therefore, the code starts to copy everything inside memory to the destination until the next \lstinline{null} byte.

\begin{lstlisting}[language=c,frame=tlrb]
00AAAAAA\00AFBFED ...
\end{lstlisting}

The sudo sanitizes every input. The function which sanitizes sudo arguments is \lstinline{parse_args}. This function adds another backsplash after special characters, thus, It is impossible to have one backslash anymore, the exploitation would not work. The attackers were looking for a way to bypass \lstinline{pars_argc}. They understood if we pass \lstinline{-s} with sudoedit instead of sudo, the \lstinline{parse_argv} does not sanitize the input argument.

The next step is to abuse the overflow vulnerability to execute arbitrary code. The researchers propose a couple of techniques for arbitrary code execution. Buffer Overflow gave the attacker ability to overwrite what's come after the buffer. The teams figure out if we overwrite the value of \lstinline{name} property inside \lstinline{servie_user} structure, in future \lstinline{nss_load_library} use this structure to load an external library with \lstinline{dlopen} which is a perfect target for any explorations. However, there is no right way to overwrite an object directly. We have no control over memory and can not determine what comes after the buffer. Different allocation and deallocation cause different heap layouts. If we overflow the heap blindly, we might overwrite other objects too and break the program execution. The researchers used the \emph{heap fang shui} to obtain the required heap layout.

\section{WhatsApp out of bounds}
\label{sect:whatsappexp}
\emph{WhatsApp} is one of the most famous instant messenger Applications in the world. They have a cross-platform instant messenger with end\-to\-end encryption and more than one billion users. The security features of these messengers make it a popular choice among those who care about message security.
A security research team has been found a vulnerability inside the \lstinline{whatsApp} image filter function ~\cite{vuln_whp}. The vulnerability trigger by applying a filter on a crafted malicious image. The researchers use afl\-fuzzer to generate different images as input files and caused WhatsApp to crash. Unlike the sudo case, the researchers do not have the source code. They captured the crash log, then did reverse engineering to identify the source of the crash. In this case, a heap base \emph{out-of-bound} was the cause of vulnerability. The problem was that both source and destination were assumed to have the same size and \lstinline{ARGB} format with 4 bytes per pixel. An attacker craft an image with 1 byte per pixel, but, the function still reads 4 bytes per pixel that causes the \emph{out-of-bound} read vulnerability.

\chapter{State of art}
Over the years, a lot of works has been done to make it more secure. In addition, many techniques have been used to exploit the weaknesses of its various implementations. In this section, we mention some of the previous work on heap exploitation. In \refToSection{heaphopper} we discuss heap hopper ~\cite{217488}  as a Framework to evaluate any heap implications, then in \refToSection{maze}, we introduced MAZE   ~\cite{272228} which is an Automatic heap fang Shui technique. 

\section{HeapHopper}
\label{sect:heaphopper}
Unlike many papers, the heap hopper authors go to the memory allocator and try to come up with a solution to check the security of the implementation of heap memory. The solution is base on symbolic execution to analyze the Security of a heap implementation. Also, we should mention heap hopper works on a heap implementation that keeps chunks metadata alongside chunks themselves like \libc{}. HeapHopper works on a compiled version of a heap manager instead of source code in this way they can analyze an implementation without source code. HeapHopper takes compiled implementation of heap manager alongside a configuration file which indicates the list of Transactions, bound, and list of Security properties. List of transaction indicate operations that can be perform by an attacker like \mallocc{}, \freec{}, \emph{use-after-free}, and \emph{buffer overflow}. Bound is the maximum number of Transactions in one attack. The list of Security properties is a list of the invalid state which an attacker can reach.The output is a proof\-of\-concept file in \lstinline{C}.Heap hooper try to find a sequence of transaction that reach a state of violated a Security property. To do that, by enumerating permutations of the transactions provided in the input file, it generates a list of transactions. Then for each sequence, a \lstinline{c} file generate and compiled. Then each file symbolically executes by a framework called \emph{angr}.In a case study, the authors test heaphopper on \lstinline{ptmalloc}. They successfully reproduce all known attacks in \lstinline{ptmalloc} except the \emph{the house of force}.

\section{Maze}
\label{sect:maze}
Many attacks on memory vulnerability need a special heap layout. As we mention in \refToSection{sudoexp} the technique used to obtain the required layout called \emph{heap fang shui}. The paper presents a novel approach to automatically manipulate a heap layout. They divide the problem into 3 phases: Generate Expected heap layout, Heap layout manipulate, create and exploit. The paper focuses on the main part: Heap layout manipulates. In the first phase, we have to make the expected heap layout base on the vulnerability we want to exploit. The author claim many sanitizers can found the vulnerability and corresponding object. The second phase is to manipulate the heap layout. Before continuing this part, I have to mention, the author presents a concept called \emph{Heap layout primitive}. The heap layout primitive is a block of code for heap layout manipulates by heap allocator inside the program. The paper converts the problem to a simple problem: Put object \lstinline{o} inside place \lstinline{p} at the time \lstinline{t}. To do that, the author first extracts and analysis Heap layout primitives. The first step is to extract the primitive, to do that, the author claims each program driven by user input has a function with a loop to handle input events. The author extracts this snippet of code with the help of static analysis and uses them as a tool to manipulate the heap layout. Of course, to have a precise heap layout we need to know the semantic of extracted primitive like the size of allocation/deallocation. The paper used \emph{taint analysis} and \emph{symbol extraction} to extract semantic data. The next step is to utilize extracted primitive in a way to manipulate heap layout. The paper present a novel algorithm call \lstinline{dig and fill}. At the time of allocation the target object \lstinline{o}, if the target place \lstinline{p} already taken by another object \lstinline{o-} then \lstinline{dig} multiple holes before \lstinline{o-} allocation by adding some of extracted primitive, to leave place \lstinline{p} for object \lstinline{o}. At the time of allocation target object \lstinline{o} if the target place \lstinline{p} is free but the allocator put \lstinline{o} inside another hole then \lstinline{fill} holes until allocator put \lstinline{o} inside \lstinline{p}. The last phase is to generate a full chain exploit base on the heap layout generated in phase 2.
...
As you can see, the heap has four chunks after three memory allocations. The first three memory chunks are requested by the user. The last chunk with the big size is the \emph{top chunk}. The top chunk is the remaining free space in the current heap. Now look at returned address by \libc{}:
...
The next question that arises is that what does happen if there are no suitable free chunks. In this situation, the heap manager checks the remaining space at the end of the heap, that is, the top chunk. The heap manager creates a fresh memory chunk from top chunk If there is enough space.

\subsection{Asking for more memory}
Assume we do not have any suitable free chunk, and the request is much bigger than the remaining space in the top chunk. In this case, the heap manager asks the Linux kernel for more memory. Now we have to discuss another system call \brkc. When a program executes, the heap manager calls \sbrkc, which itself used the \brkc system call. This system call allocates memory just after the program gets loaded.
...
\subsection{\Fb}
\Fb{}\footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=ee87ddbbf9c3b8da9f0f6daa84b42b0a6a859fe0;hb=3de512be7ea6053255afed6154db9ee31d4e557a\#l1564}{\texttt{malloc.c}, line 1564}} is an optimization on top of the previously mentioned bins. Each bin keeps recently freed chunks in a single-linked list. We have ten \fb{}, each of them kept just one chunk size. Chunks in \fb{} do not merge with neighbors; instead, \fb{} keeps them alive, as a result, if the program sends the same size request, the heap manager can use these chunks.
...
We worked exclusively on \libc{} as the heap manager on the x86 machine, so the primary tool is \emph{gdb}~\cite{gdb}, as explained in \refToSection{GDB}. In addition to gdb, we use some utilities like its extension \emph{GEF}~\cite{gef}, introduced in \refToSection{GEF}, and \emph{heappy}~\cite{heappy}, discussed in \refToSection{heappy}, to have better memory visualization. \emph{pwndbg}~\cite{pwndbg} , introduce in \refToSection{PWNDBG} , to improve gdb visualization , \emph{Libc Database}~\cite{libcdatabase}, discuss in \refToSection{LibcDatabase}, to have all necessary versions of \libc{} and related files. \emph{Patchelf}~\cite{patchelf}, discuse in \refToSection{Patchelf} to modify the dynamic loader and RPATH of executable files, also in \refToSection{DebugLibc} we discuss how to debug executable and library inside the gdb.
...
\subsubsection{heap}
Gef has a lot of debugging tools; we focus on the heap utility of gef which is call \emph{heap}. The heap command provides us information on \libc{} heap memory allocation. Syntax to the subcommands is straightforward:
\begin{lstlisting}[frame=tlrb]
...
This command provides information on a chunk like the size and pointer. Provide the address of the chunk data section to the command.
\begin{lstlisting}[frame=tlrb]
...
As we mentioned in \refToSection{subheaps}, a multi-thread application has many arenas, not just one. Gef proves a sun command to retrieve the information of all arena.
...
There is a loop inside the \lstinline{main} function that prints two pointers. The first one \lstinline{auth} is a pointer to a structure, the second one \lstinline{service} is a char pointer to a string. In the following, we have a secure \lstinline{fgets} function that reads a line from the input. After that, we had four \lstinline{if} statements that compare the input by the following command.

As we can see, the \lstinline{auth} command allocate the object , clear current data inside object , then copy whats come after \lstinline{auth} command to the \lstinline{auth->name}. The \lstinline{login} command checks if we have a value in \lstinline{auth->auth}, it means that the user is logged in, otherwise it asks for the password. Our goal is set any value inside \lstinline{auth->auth}.

Despit the \lstinline{free} command release the \lstinline{auth} object, The pointer still points to the memory location,so , the next \lstinline{login} command still can use it. There is where \emph{use after free} occure.
...
What happens if an allocated chunk of data gets freed twice. Such a scenario leads to a corrupted heap data structure that an attacker can leverage. As we discussed before, A chunk is added to the corresponding bin after release by the user. In the simplest scenario in \libc{-2.23}, it is added to \fb{}. Consider the following sample code:
...
At first, we requested 3 chunks of 10-bytes each. After that, we release \lstinline{a} and \lstinline{b} then, \lstinline{a} again. You can see the fast bin structure after \lstinline{free} below step by step:
...
We have \lstinline{a} twice in the \fb{} now.
The question may be raised why we freed \lstinline{b} in the middle. Releasing twice \lstinline{a} in a row activates one of the security checks \fb{}. After the first memory release, the \fb{} can detect that \lstinline{a} is at the top of the list now and end the program by sending an error. To bypass these security checks, it is mandatory to free another chunk in the middle. Then, three new chunks of data are allocated in the following. You can see the fast bin state below step-by-step during the second allocation:
...
Currently, this method needs to be modified since \libc{-2.26} introduced the \tch{}. On the new \libc{}, freed chunks are added to the \tch{} instead of \fb{}, as a result, we need to do some modifications to this attack. Consider the following code:
...
A simple solution to add free chunks to \fb{} instead of \tch{} is to fill the \tch{}, in the following, create new chunks from the top chunk instead of returning freed \tch{} chunks. The above code allocates seven chunks of 8 bytes, then free all of them. In the new version of \libc{}, all of these seven freed chunks are added to the \tch{}. The \Tch{} can keep up to 7 chunks. From this point, if we release other chunks, they are added to \fb{} because \tch{} is full. Now we pull back to the original approach and allocate three new chunks. If we use \mallocc{} to allocate fresh chunks, the heap manager returns chunks from \tch{}; so, \tch{} becomes accessible again, and we can not implement the attack. In such a situation an attacker could leverage \callocc{}. As we mention in \refToSection{calloc} since \callocc{} does not use \tch{}, and \fb{} are free, so \callocc{} gets memory from the top chunk.
...
The third \mallocc{} asks for a significant memory allocation; this allocation triggers another method call \lstinline{malloc_consolidate} as we discuss in \refToSection{mallocconsolidate} . \Mallocc{} in this situation, consolidate chunks in the \fb{} before continuing, then put the resulting merged chunks on the \ub{}.
Such behavior is that we have another solution to bypass the \fb{} double-free security check. It is possible to use \mallocc{} with a big size instead of freeing another chunk in the middle of two free. The third \mallocc{} moves a chunk from \fb{} to the \ub{}; as a result, when the second free request arrives, there is no chunk inside the \fb{}. We can implement a \emph{double-free} attack. In the final layout, we have \lstinline{p1} on both \fb{} and \ub{}. Moving \fb{} chunks to \ub{} by asking for a big size memory allocation is a simple approach used by other attack methods too.

\section{Forged Chunk}
\label{sect:forgedchunk}
Freeing up a chunk does not prevent access to its memory location because the pointer still exists in the application, as a result, we can implement an attack on the heap data structure by controlling the pointer. The code below shows how we can return our forged chunk instead of a real heap chunk:
...
In the first step, we allocate a memory \lstinline{a} with 10-byte size, also, create our fake chunk with size 0x20. Chunk size is essential. It must fall in the \fb{} range because \fb{} has a security check on chunk size while removing it from the list.
...
In the following, by first allocating \lstinline{a} again, the heap manager returns the \lstinline{a} from the \fb{}; following that, our forged chunk will be at the head, so a subsequent allocation returns it instead of a real chunk.
...
Taking this into consideration, another allocation on \fb{} could raise a \emph{segmentation fault}.

In the next sample, we use \emph{double-free} vulnerabilities in \libc{-2.23}; the below code is just an extended version of the \emph{double-free} attack which we examined. The extended version tricks \mallocc{} into returning a controlled memory location on the stack instead of a real chunk of the heap.
...
This code tries to write an address inside the heap area in the stack. In \refToSubSection{houseofroman}, we talk about using this type of vulnerability in a real attack.
...
Now it is clear that if we control the backward pointer, then, we can write data to any address, however, we do not have control over data. In the case of \emph{ASLR}, this vulnerability can help to find an address inside the heap area.
...
The last step is to allocate a bigger chunk, so, \lstinline{p2} moves to the \lb{} . When \lstinline{p2} move to \lb{}, the heap manager write the address of \lstinline{p2} inside \lstinline{p1->bk_nextsize->fd_nextsize} as the code of \libc{} show \footnote{\href{https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c;h=f7cd29bc2f93e1082ee77800bd64a4b2a2897055;hb=9ea3686266dca3f004ba874745a4087a89682617\#l3843}{\texttt{malloc.c}, line 3843}}:
...
This attack works on current versions of \libc{} too.

\section{Overlapping chunks}
\label{sect:overlappingchunk}
The goal is to have two chunks, writing data in one of them and overwriting the other one data. We discuss this attack on a sample, first, we allocate 3 chunks of memory:
...
Chunk(addr=0x555555559210, size=0x80, flags=)
 ...
 After release the second chunk, As we discuss, This chunk adds to the \ub{} at first, then, It can be used for future allocation:
...
As you remember, currently, \lstinline{p2} is located at the \ub{}, also, we change the size of \lstinline{p2}, so, the heap manager return \lstinline{p2} when receives an allocation request, as the result, \lstinline{p2} has overlap with the previous chunks:
...
As you can see, \lstinline{p3} is not in the list of chunks anymore, however, the pointer is still accessible inside the application. Now the \lstinline{p3} and \lstinline{p4} have overlap, so, write data on one of them overwrite the other one data
...
Like the previous one, we change the size of the last chunk. The new chunk size is the sum of the size of \lstinline{chunk_3} and \lstinline{chunk_2}; as a result, the unmmap release both areas.

\begin{lstlisting}[language=c,frame=tlrb]
free(mmap_chunk_3)
 \end{lstlisting}

In this stage, if we allocate a new big chunk, this new big chunk has overlap with \lstinline{chunk_2} which just has been unmmap without call unmmap directly; as a result, we can write on the \lstinline{chunk2} area with the help of our new chunk.

\section{The Houses}
\label{sect:thehouses}
It is time to talk about the \emph{House of XXX} families attack. In 2004 the authors of \libc{} published a series of patches over \mallocc{} to prevent previous attacks on heap manager. In late 2004 a person issued a paper \emph{The Malloc Maleficarum}\footnote{\href{http://phrack.org/issues/66/10.html}{\texttt{The Malloc Maleficarum}}}, over the internet. ``It is for this reason, a small suggestion of impossibility, that I present the Malloc Maleficarum'' As they said. The name came from a 14-century book written by a Catholic clergyman on witchcraft.
...
The fake arena is located in an offset of the heap, as a result, we need to allocate chunks until reach the fake arena.
\begin{lstlisting}[language=c,frame=tlrb]
...
As we told, this approach uses the \fb{} approach, so, we need a chunk in range of the \fb{} chunk, also, a pointer to chunk:

\begin{lstlisting}[language=c,frame=tlrb]
uint64_t* fastbin_chunk = malloc(0x50)
uint64_t* chunk_ptr = fastbin_chunk - 2
\end{lstlisting}
This file work on \libc{-2.31} which support \tch{} as the first mechanism, so, like \emph{double free} in \refToSection{doublefree}, all we need is to fill up the \tch{} to force the heap manager role back to the \fb{}
...
\end{lstlisting}
As you can see in the first step \libc{} compares the requested chunk size and top chunk size, as the result, the first step is to set a large value in top chunk size to bypass this step, cheating \libc{} to use this space. The following code Is \lstinline{hose_of_force} sample from how2heap\footnote{\href{https://github.com/shellphish/how2heap/blob/master/glibc_2.23/house_of_force.c}{\texttt{house\_of\_force.c}}}.
...
This chunk points to our target, so, with one simple \lstinline{strcpy} we can overwrite the value:
...
As we can see, if we have control over the victim \lstinline{bck} pointer and bypass the security check, \libc{} returns our fake chunk on the next \sbs{} allocation. In other words, we can return a chunk to any address. Now we explain it in a real sample from how2heap\footnote{\href{https://github.com/shellphish/how2heap/blob/master/glibc_2.23/house_of_lore.c}{\texttt{house\_of\_lore.c}}}:
...
\end{lstlisting}
Now we forge a fake chunk, point the \lstinline{bck} pointer of \lstinline{buffer_1} to the victim, the \lstinline{fwd} pointer of \lstinline{buffer_1} to \lstinline{buffer_2}, also, the \lstinline{bck} pointer of \lstinline{buffer_2} to \lstinline{buffer_1}. In this way, we can bypass the \sbs{} security check.
...
To prevent merging the top chunk with a small one during \freec{} we allocate a big chunk on top of the heap. After that by release the victim, it moves to the \ub{} as shown in Figure~\ref{fig:gdb2}. In the following step, we allocate another chunk big enough, so, heap manager can use the chunk inside an \ub{} or \sbs{}.
\begin{lstlisting}[language=c,frame=tlrb]
...
\begin{figure}[h!]
 ...
 \label{fig:gdb3}
\end{figure}
The victim is not in the \ub{} anymore; instead, the heap manager moves it to the \sbs{} after allocation of \lstinline{p2} ,we can see the result in Figure~\ref{fig:gdb3}. As we mention in \refToSection{malloc}, the heap manager checks all of the chunks inside the \ub{}. If the size is not suitable for the current allocation, they move to the corresponding bin. In other words, we force the heap manager to move the victim from the \ub{} to the \sbs{}.
...
The result of such an action is, our fake chunk is at the top of the bin, so the next \mallocc{} returns to the attacker's controlled location.

\subsection{The House of Orange}
\label{subsect:houseorange}
The house of orange is different from previous attacks. As we remember, in the previous attack, we use \mallocc{} and free to abuse libc vulnerability, but, In the house of orange, we do not use the \freec{} function at all. In other words, we force the heap manager to free a chunk of the heap without calling the \freec{} function. The final layout of this attack is a combination of \ub{} attacks. The POC file is available in the how2heap repository\footnote{\href{https://github.com/shellphish/how2heap/blob/master/glibc_2.23/house_of_orange.c}{\texttt{house\_of\_orange.c}}}.
...
Now, we have to change the top chunk size in a way to meet security checks. By set 0xc00 + \lstinline{Prev_flag}, we can bypass the security check. At least, we request a chunk more significant than the top chunk size. In this situation, the heap manager creates a new top chunk where the old one has ended, then calls the free function to release the old top chunk, so the old one moves to an \ub{}.

The first part of the attack finished, in the following, It's time to talk about \libc{} file structure and \emph{abort} routine before continuing to the second phase. What's happen if \libc{} detected corruption inside the heap structure. When libc detects any memory corruption, it calls the \emph{abort} routing automatically. \emph{Abort} routine flush all file pointers in memory by calling \lstinline{_IO_flush_all_lockp}. Consider a part of code:
...
During iteration over chunks inside the \ub{}, the libc checks the size of the chunk; if the size is smaller than \lstinline{MINSIZE}, then the abort procedure is called, and an attack happens.
...
This attack used a \Fb{} and \emph{uaf} attack to forge a fake heap chunk for feature uses. As we mentioned before, every fast bin keeps a single size chunk with a single linked list. What happens if we change a chunk inside the \fb{}? In allocation, \fb{} detects the abnormal size so aborts the allocation. If we force the heap manager to call \mallocc{} in order to consolidate, this chunk merge and split into \sbs{}. As a result, we have two valid but forged chunks in a \sbs{} which overlap which other. Consider the following code:
...
In this attack, the goal is to force \mallocc{} to return a chunk in any given address by use of the \freec{} function. Consider the free function:
...
As we mention, we need to fill up \tch{}, so, we perform the \freec{} function on the 7 chunks, as the result, heap manager forces to use the \ub{} and other bins:
...
Now the \tch{} is full. We perform the free function on the \lstinline{prev} and \lstinline{a}.
...
The result of the above is to consolidate the \lstinline{prev} chunk with the victim \lstinline{a} as shown in Figure~\ref{fig:gdb11}. Attention to the size and the address of the chunk:
...
In the next step, with the help of \emph{use after free} and \emph{double-free} attacks, we put back the victim in the \tch{}
...
\end{lstlisting}
In the final step with one allocation, we can remove the top chunk from the \tch{}, so, the next allocation returns our controller location (the size must be the same):
\begin{lstlisting}[language=c,frame=tlrb]
malloc(128)
...
\chapter{Conclusion}
We have discussed heap implementation and various types of attacks on heap memory allocation. As we have seen, the heap is a very complex memory that must use carefully by the programmers. Through decades various patches and optimization have been implemented over the heap, however, the attack is still possible.

The main subject of this thesis consists of the survey of \libc{} heap exploitation in detail. We discussed obstacles, security checks; and steps in one or more versions of Glibc for each exploitation. Because knowing how \libc{} works helps to understand the exploitation; We discuss \libc{} memory allocation methods in detail step by step; also, we illustrated how \libc{} manages free memory space.
