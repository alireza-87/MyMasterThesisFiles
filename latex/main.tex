...
We worked exclusively on \libc{} as the heap manager on the x86 machine, so the primary tool is \emph{gdb}, as explained in \refToSection{GDB}. In addition to gdb, we use some utilities like its extension \emph{GEF}, introduced in \refToSection{GEF}, and \emph{heappy}, discussed in \refToSection{heappy}, to have better memory visualization.\glnote{added references to (sub)sections}

\glnote{you shoud list all sections here: heappy, libc db, patchelf, \ldots}
...
\glnote{I'd use paragraph* (that is, with the star) to avoid something like 3.1.1.1.1}
...
\glnote{I'd remove the floating figures/references and simply include the graphics between begin-center/end-center, see the latex}
% \begin{figure}[h!]
...
This command provides information on a chunk like the size and pointer. Provide the address of the chunk data section to the command.``Figure~\ref{fig:gefheapchunk}``\glnote{for closing quotes you should use two single-quote characters: ''; please change everywhere}
\begin{lstlisting}[frame=tlrb]
...
As we mentioned\glnote{this is mentioned in another chapter, in cases like this, it's better to add a reference to the other section}, a multi-thread application has many arenas, not just one. Gef proves a sun command to retrieve the information of all arena.``Figure~\ref{fig:gefheaparena}``
...
\subsection{PWNDBG}\glnote{this tool is not mentioned at the beginning of the chapter! Please add it}
...
\glnote{same thing as before, regarding the figures}
\begin{figure}[h!]
...
Now, all we have to do is remove \lstinline{a} from the top of the \fb{} with a call to \mallocc{}, then, the final allocation returns a controlled address. In this case, a pointer into the stack.\glnote{I rephrased, please check if this is what you meant}
...
Let's start with an example and consider the following code. This code tries to write a big value in the stack:\glnote{I don't understand in which sense is trying to write ``a big'' value\ldots}
...
Now it is clear that if we control the backward pointer, then, we can write data to any address, however, we do not have control over data. In this case, the linked list header was written on a stack variable.\glnote{this could be useful to defeat ASLR! You should say so}
